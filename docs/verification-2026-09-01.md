...
# Cycle D Task 5, third attempt — built from Rock's own brief

The first two attempts were me reading a reference image and getting it wrong.
This one is his written brief, followed item by item.

**1. Surface and canvas depth.** Canvas `#0b0c10`. Panels `#121318`, one solid
surface with the controls resting directly on it — no nested boxes.

**2. Border and line reduction.** Painted border edges in the running app:

| | edges |
|---|---|
| before Cycle D | 148 |
| after the tile/reset/section pass | 81 |
| **now** | **0** |

Every input, control and container is a fill. The outlines that framed them
are gone, and so is the framing line under the toolbar, beside the rail and
down each panel's edge — the panel's own colour does that work.

Removing `border` from a `<button>` brings the browser's default `2px outset`
back, which drew a box around every row. Twelve rules needed `border: none`
said outright. Caught by measuring the rendered borders, not by reading the
stylesheet.

**3. Contrast and typography.** Two text tokens: `#ffffff` and `#c9cdd6`.
`--text-muted` is gone — "upgrade all muted gray text to crisp silver and pure
white" leaves two. Section titles are the silver, unboxed.

**4. Controls.** Segmented tracks are `#252831` with a solid `#454b57` active
pill and white ink — no outline. Chips, the search field, the steppers and the
selected row use the same two fills. The slider track is 6px and smooth, the
handle 16px and round.

`--surface-window` → `--surface-panel` is **1.054:1**, below the 1.2 step the
ladder guard requires. That is Rock's own pair of values, and the separation
is not meant to come from the surfaces — it comes from the outlines being
gone. The guard exempts that one pair by name rather than silently.

509 tests pass. No console errors, no horizontal scroll.

## Task 5, third attempt — fix round: it is actually white now

Rock inspected five elements in dev tools. Every one came back `#C9CDD6`:
"Hue", "1800×1200", "4:3", "Corner radius", "Lavender". *"is this showing a
white element to you?"* No. The app defined `--text-primary: #ffffff` and then
pointed **43 usages** at the silver token instead, so white existed and was
not used.

`--text-secondary` is retired. **There is one text colour: `#ffffff`.** Off
states still step down to `--text-inert`, which is a state rather than a rung.

Measured after, in the running app:

| element | colour | size |
|---|---|---|
| section heading | `rgb(255,255,255)` | 15px |
| size row | `rgb(255,255,255)` | 13px |
| dimensions | `rgb(255,255,255)` | 11.5px |
| slider label | `rgb(255,255,255)` | 13px |
| preset tile label | `rgb(255,255,255)` | 11.5px |
| tab | `rgb(255,255,255)` | 15px |

**And the control from screen011.** The segmented control was a joined track
with a filled pill. It is now four separate rounded rectangles with gaps, each
40px tall with its own fill, and the chosen one carries a periwinkle edge
(`--accent: #8b8ff0`) rather than a highlight fill. Chips follow the same
idiom one size down. Section headings lost the all-caps and the letter
spacing: "Background", not "BACKGROUND".

**The two archaeology assertions finally had to go.** They reconstructed a
pre-Cycle-A opacity failure from live tokens; with a single white rung, a 0.4
composite of white lands at 3.79:1 — brighter than the inert tone — so the
reconstruction proved the opposite of what it was written for. Replaced with
the property: an off state is under half the live ink and still clears 3:1.

506 tests pass. No console errors.
