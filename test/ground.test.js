...
describe('groundFor ground swatches (hex output)', () => {
  ...
  //
  // WHAT THIS TEST PROVES CHANGED IN CYCLE C TASK 3, and it is worth being
  // precise about because it got weaker.
  //
  // The values used to come from ground.py's own hsl() helper
  // (colorsys.hls_to_rgb, not this JS port), evaluated at sat=0.16 - so
  // they proved core/ground.js agreed with the Python original's actual
  // formula rather than merely with itself. Task 3 raised the saturation
  // formula deliberately, which breaks that agreement on purpose: the
  // shipped palette was so flat that two of the eight named grounds
  // differed by ONE level in their largest channel. ground.py itself was
  // retired in Cycle A, so the agreement was already historical; this is
  // the commit that actually ends it.
  //
  // These values are now generated from core/ground.js, so what remains is
  // a freeze, not a cross-check: it catches the hex output drifting
  // silently, which is real - the golden-image test above reads only
  // hue/lum/chroma/darkUI and would never notice - but it can no longer
  // catch this file disagreeing with an external reference, because there
  // is no longer an external reference.
  //
  ...
  const GOLDEN_SWATCHES = {
    0:     { mid: ['#ded6d6', '#cdc1c1', '#beadad'], light: ['#faf8f8', '#efe9e9', '#e3d8d8'] },
    15:    { mid: ['#ded8d6', '#cdc4c1', '#beb1ad'], light: ['#faf8f8', '#efeae9', '#e3dad8'] },
    90:    { mid: ['#daded6', '#c7cdc1', '#b6bead'], light: ['#f9faf8', '#ecefe9', '#dde3d8'] },
    180:   { mid: ['#d6dede', '#c1cdcd', '#adbebe'], light: ['#f8fafa', '#e9efef', '#d8e3e3'] },
    217.3: { mid: ['#d6d9de', '#c1c5cd', '#adb4be'], light: ['#f8f8fa', '#e9ebef', '#d8dce3'] },
    268:   { mid: ['#dad6de', '#c7c1cd', '#b5adbe'], light: ['#f9f8fa', '#ece9ef', '#ddd8e3'] },
    300:   { mid: ['#ded6de', '#cdc1cd', '#beadbe'], light: ['#faf8fa', '#efe9ef', '#e3d8e3'] },
    333.2: { mid: ['#ded6da', '#cdc1c6', '#beadb5'], light: ['#faf8f9', '#efe9ec', '#e3d8dd'] },
    359:   { mid: ['#ded6d6', '#cdc1c1', '#beadad'], light: ['#faf8f8', '#efe9e9', '#e3d8d8'] },
    359.9: { mid: ['#ded6d6', '#cdc1c1', '#beadad'], light: ['#faf8f8', '#efe9e9', '#e3d8d8'] },
  };
